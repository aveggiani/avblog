...
<cfswitch expression="#attributes.type#">
	...
	<cfcase value="show">
		...
		<cfelse>
			...
			<cfhtmlhead text="<link rel=""microsummary"" href=""#request.blogmapping#microsummary.cfm?id=#attributes.id#"">">
		</cfif>
						...
						<cf_cache action="#mycaching#" name="#arrayShow[i].id#" timeout="#request.cachetimeout#">		
							...
							<cfoutput>
								<!--
									...
									<rdf:Description
										rdf:about="http://#cgi.SERVER_NAME##request.blogmapping#index.cfm?mode=viewEntry&id=#arrayShow[i].id#"
										dc:identifier="http://#cgi.SERVER_NAME##request.blogmapping#index.cfm?mode=viewEntry&id=#arrayShow[i].id#"
										dc:title="#arrayShow[i].title#"
										trackback:ping="http://#cgi.SERVER_NAME##request.blogmapping#trackback.cfm?id=#arrayShow[i].id#" />
									</rdf:RDF>
								-->
								...
								<div class="#blogClass#">
									...
									<div id="enclosures_#request.indexBlog#" class="enclosures" style="display:none">
										...
										<cfloop query="qryEnclosures">
											<a href="#request.blogmapping#user/enclosures/#qryEnclosures.name#">#qryEnclosures.name#</a> (#qryEnclosures.length# bytes - #qryEnclosures.type#)
											<br />
										</cfloop>
									</div>
									<cfif listlen(myCategories) gt 0>
										<div class="blogCategories">
											...
											<cfloop index="item" list="#myCategories#">
												<a href="#request.blogmapping#permalinks/categories/#listrest(item,'_')#">#listrest(item,'_')#</a>
									...
									<div class="blogCommands">
										...
										<cfif howmanycomments is not 0 and not isdefined('url.viewcomment')>
											<a href="#request.blogmapping#index.cfm?mode=viewcomment&amp;id=#urlencodedformat(arrayShow[i].id)#">#howmanycomments# #iif(howmanycomments EQ 1, DE(application.language.language.comment.xmltext), DE(application.language.language.comments.xmltext))#
										...
										<a href="#request.blogmapping#index.cfm?mode=addcomment&amp;id=#urlencodedformat(arrayShow[i].id)###addcomment">#application.language.language.addcomment.xmltext#</a>
										<cfif application.configuration.config.options.trackbacks.xmltext>
											<vb:icon type="trackback">
											<a href="#request.blogmapping#index.cfm?mode=addtrackback&amp;id=#urlencodedformat(arrayShow[i].id)#">#application.language.language.addtrackback.xmltext#</a>
											<cfif howmanytrackbacks is not 0 and not isdefined('url.viewtrackback')>
												<a href="#request.blogmapping#index.cfm?mode=viewtrackback&amp;id=#urlencodedformat(arrayShow[i].id)#">#howmanytrackbacks# #application.language.language.trackback.xmltext#</a>
											</cfif>
										</cfif>
									</div>
									<cfif isuserinrole('admin') or (isuserinrole('blogger') and arrayShow[i].author is listgetat(GetAuthUser(),1))>
										<div class="blogAdmin">
											[<a href="#request.blogmapping#index.cfm?mode=deleteentry&amp;id=#urlencodedformat(arrayShow[i].id)#">#application.language.language.delete.xmltext#</a>] 
											[<a href="#request.blogmapping#index.cfm?mode=updateentry&amp;id=#urlencodedformat(arrayShow[i].id)#">#application.language.language.edit.xmltext#</a>]
										</div>
									</cfif>
								</div>
								<cfif not arrayShow[i].published>
