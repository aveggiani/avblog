<cfcomponent>
	...
	<cffunction name="getAllCount" output="false" returntype="numeric">
		<cfscript>
			qryVerify = application.fileSystem.getDirectoryxml('#request.BlogPath#/#request.xmlstoragepath#/trackbacks','name','*');
		</cfscript>			
        ...
        <cflock timeout="10" throwontimeout="Yes" name="#arguments.id#" type="EXCLUSIVE">
			<cfscript>
				if (isdefined('arguments.id'))
					...
					filter = "*";
				qryVerify = application.fileSystem.getDirectoryxml('#request.BlogPath#/#request.xmlstoragepath#/trackbacks','datelastmodified desc',filter);
			...
			<cfif qryVerify.recordcount gt 0>
				<cfloop query="qryVerify" startrow="#rstart#" endrow="#rend#" >
					<cfscript>
						if (fileexists('#request.BlogPath#/#request.xmlstoragepath#/trackbacks/#qryVerify.name#'))
							{
								xmltrackback = application.fileSystem.readXml('#request.BlogPath#/#request.xmlstoragepath#/trackbacks/#qryVerify.name#');
	...
	<cffunction name="save" output="false" returntype="void">
		...
		<cfscript>
			application.fileSystem.writexml('#request.BlogPath#/#request.xmlstoragepath#/trackbacks','#arguments.structTrackBack.id#_#arguments.structTrackBack.blogid#','#item#');
		</cfscript>
	...
	<cffunction name="delete" output="false" returntype="void">
		...
		<cfscript>
			qryVerify = application.fileSystem.getDirectoryxml('#request.BlogPath#/#request.xmlstoragepath#/trackbacks','','#arguments.id#_*');
		</cfscript>
		<cfif qryVerify.recordcount gt 0>
