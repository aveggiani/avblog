<cfcomponent>
	...
	<cffunction name="loaddates" output="false" returntype="string">
		...
		<cfscript>
			...
			caricaxml = application.fileSystem.getDirectoryxml('#request.appPath#/#request.xmlstoragepath#/entries','name desc');
		</cfscript>
	...
	<cffunction name="show" output="false" returntype="array">
		...
		<cfscript>
			qryDayFiles = application.fileSystem.getDirectoryxml('#request.appPath#/#request.xmlstoragepath#/entries','name desc','#filter#');
		</cfscript>
		...
		<cfloop query="qryDayFiles">
			<cfif (not arguments.isAdmin and 
				...
				<cfscript>
					tmpTxt = application.fileSystem.readxml('#request.appPath#/#request.xmlstoragepath#/entries/#qryDayFiles.name#');
	...
	<cffunction name="getFromPermalink" output="false" returntype="uuid">
		...
		<cfscript>
			qryVerify = application.fileSystem.getDirectoryxml('#request.appPath#/#request.xmlstoragepath#/entries','name desc','#arguments.date#_*');
		</cfscript>
		
		<!---
		<cfdirectory action="list" name="qryVerify" directory="#request.appPath#/#request.xmlstoragepath#/entries" filter="#arguments.date#_*">
		--->

		<cfif qryVerify.recordcount is 1>
			<cfscript>
				xmlBlog = application.fileSystem.readxml('#request.appPath#/#request.xmlstoragepath#/entries/#qryVerify.name#');
	...
	<cffunction name="get" output="false" returntype="struct">
        ...
        <cflock timeout="10" throwontimeout="Yes" name="#arguments.id#" type="EXCLUSIVE">
			<cfdirectory action="list" name="qryVerify" directory="#request.appPath#/#request.xmlstoragepath#/entries" filter="*_#arguments.id#.xml">
			<cfif qryVerify.recordcount gt 0>
				<cffile charset="#request.charset#" action="READ" file="#request.appPath#/#request.xmlstoragepath#/entries/#qryVerify.name#" variable="xmlBlog">
			</cfif>
        </cflock>
			--->

		<cfscript>
			qryVerify = application.fileSystem.getDirectoryxml('#request.appPath#/#request.xmlstoragepath#/entries','name desc','*_#arguments.id#');
	...
	<cffunction name="getRecent" output="false" returntype="query">
		...
		<cfscript>
			...
			qryBlogsDirectory = application.fileSystem.getDirectoryxml('#request.appPath#/#request.xmlstoragepath#/entries','name desc','????????_??????_true_*');
	...
	<cffunction name="delete" output="false" returntype="void">
        ...
        <cflock timeout="10" throwontimeout="Yes" name="#arguments.id#" type="EXCLUSIVE">
			...
			<cfscript>
				qryVerify = application.fileSystem.getDirectoryxml('#request.appPath#/#request.xmlstoragepath#/entries','','*_#arguments.id#');
			</cfscript>
			<cfif qryVerify.recordcount gt 0>
				<cfloop query="qryVerify">
					<cfscript>
						application.fileSystem.deleteFile('#request.appPath#/#request.xmlstoragepath#/entries/#qryVerify.name#');
					</cfscript>
				</cfloop>
			</cfif>
			<cfscript>
				qryVerify = application.fileSystem.getDirectoryxml('#request.appPath#/#request.xmlstoragepath#/comments','','*_#arguments.id#');
			</cfscript>
			<cfif qryVerify.recordcount gt 0>
				<cfloop query="qryVerify">
					<cfscript>
			...
			<cfscript>
				qryVerify = application.fileSystem.getDirectoryxml('#request.appPath#/#request.xmlstoragepath#/trackbacks','','*_#arguments.id#');
			</cfscript>
			<cfif qryVerify.recordcount gt 0>
				<cfloop query="qryVerify">
					<cfscript>
			...
			<cfif fileexists('#request.appPath#/storage/xml/subscriptions/#arguments.id#.cfm')>
				<cfscript>
					application.fileSystem.deleteFile('#request.appPath#/storage/xml/subscriptions/#arguments.id#.cfm');
				</cfscript>
			</cfif>
        </cflock>
		...
		<cftry>
			...
			<cfindex collection="#application.applicationname#"
				...
				extensions="cfm"
				type="path"
				key="#request.appPath#/#request.xmlstoragepath#/entries">
			<cfcatch>
			</cfcatch>
		</cftry>
	...
	<cffunction name="save" output="false" returntype="void">
		...
		<cfscript>
			qryVerify = application.fileSystem.getDirectoryxml('#request.appPath#/#request.xmlstoragepath#/entries/','','*_#id#');
		</cfscript>
		<cfif qryVerify.recordcount gt 0>
			<cfloop query="qryVerify">
				<cfscript>
		...
		<cfscript>
			application.fileSystem.writexml('#request.appPath#/#request.xmlstoragepath#/entries/','#dateFile#_#timefile#_#arguments.published#_#id#',item);
		</cfscript>
	...
	<cffunction name="search" output="false" returntype="query">
		...
		<cftry>
			...
			<cfloop query="qrySearch">
				...
				<cfscript>
					qryBlog = application.fileSystem.getDirectoryxml('#request.appPath#/#request.xmlstoragepath#/entries','','*_true_#appString#');
				</cfscript>
				<cfif qryBlog.recordcount gt 0>
					<cfif (not arguments.isAdmin and 
						...
						<cfscript>
							xmlBlog = application.fileSystem.readxml('#request.appPath#/#request.xmlstoragepath#/entries/#qryBlog.name#');
							xmlBlog = xmlparse(xmlBlog);
