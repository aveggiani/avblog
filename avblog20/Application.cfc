<cfcomponent output="false">
	...
	<cfscript>
		...
		this.name 							= "AVBlog20Alpha101_#hash(request.BlogPath)#";
	...
	<cffunction access="public" name="initRequest" output="true" returnType="void">
		<cfscript>
			var path_translated 		= replace(cgi.PATH_TRANSLATED,'\','/','ALL');
			var firstlevelappfolders	= '.svn,avuserblogs,cache,cfc,config,css,customtags,external,feed,gateway,images,include,js,languages,personal,plugins,pods,skins,storage,user';
			var firstlevelblogfolders	= '.svn,cache,cfc,config,css,customtags,external,feed,gateway,images,include,js,languages,personal,plugins,pods,skins,storage,user';
			...
			request.appMapping				 	= initRequestappMapping(firstlevelappfolders);
			request.BlogMapping					= initRequestappMapping(firstlevelblogfolders);
			request.appPath					 	= initRequestappPath(request.appMapping,firstlevelappfolders);
			request.blogPath					= initRequestappPath(request.appMapping,firstlevelblogfolders);

			if (request.blogPath contains 'avuserblogs')
				{				
					request.userBlog			= true;
					request.cfcMapping			= '.';
				}
			else
				{				
					request.userBlog			= false;
	...
	<cffunction access="private" name="initRequestappPath" output="true" returnType="string">
		...
		<cfargument name="firstlevelfolders" required="yes">
	...
	<cffunction access="private" name="initRequestappMapping" output="true" returnType="string">
		<cfargument name="firstlevelfolders" required="yes">
		...
		<cfscript>
			...
			if (appMapping is '/' and path_translated contains 'avuserblogs')
				appMapping = '';
		</cfscript>
		<cfreturn appmapping>
	</cffunction>
