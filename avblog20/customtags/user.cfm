			...
			<div class="editorList">
				<table border="0" cellspacing="0" cellpadding="0" id="movableTable">
					<cfloop query="application.users">
						<cfoutput>
							<tr>
								<td>&nbsp;[<vb:wa href="#request.blogmapping#index.cfm?mode=users&amp;updateuser=#application.users.id#">#application.language.language.edit.xmltext#</a>] [<a href="index.cfm?mode=users&deleteuser=#application.users.id#">#application.language.language.delete.xmltext#</a>] #application.users.fullname#</td>
							</tr>
						</cfoutput>
					</cfloop>
				</table>
			...
			<form id="theForm" name="theForm" action="<cfoutput>#cgi.script_name#</cfoutput>?mode=users" method="post">
				<table width="100%">
					<cfoutput>
						...
						<tr>
							<td align="right">#application.language.language.user_personalblog.xmltext#</td>
							<td>
								<input type="checkbox" name="personalblog" <cfif isdefined('url.updateuser') and qryMyUser.personalblog is 'true'>checked</cfif> >
							</td>
						</tr>
						<tr>
							<td align="right">#application.language.language.user_blogaddress.xmltext#</td>
							<td><input type="text" size="50" name="blogaddress" <cfif isdefined('url.updateuser')>value="#qryMyuser.blogaddress#"</cfif> class="editorForm" /></td>
						</tr>
						<tr>
