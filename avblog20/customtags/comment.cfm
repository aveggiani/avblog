							...
							<cfif not comments.published>
								...
								<div <cfif comments.private is 'false'>class="commentBody"<cfelse>class="commentBodyPrivate"</cfif>>
									...
									<div class="commentDate">#right(date,2)# #lsdateformat(createdate(2000,mid(date,5,2),1),'mmmm')# #left(date,4)# #comments.time#
										[<vb:wa href="#request.blogmapping#index.cfm?mode=deletecomment&amp;idcomment=#urlencodedformat(comments.id)#&amp;id=#urlEncodedFormat(comments.blogid)#&amp;allcomments=1&amp;start=#url.start#&amp;now=#GetTickCount()#">#application.language.language.delete.xmltext#</vb:wa>]
										[<vb:wa href="#request.blogmapping#index.cfm?mode=viewcomment&amp;idcomment=#urlencodedformat(comments.id)#&amp;id=#urlEncodedFormat(comments.blogid)#&amp;publish=#comments.published#&amp;allcomments=1&start=#url.start#&amp;now=#GetTickCount()#"><cfif comments.published>#application.language.language.notpublished.xmltext#<cfelse>#application.language.language.published.xmltext#</cfif></vb:wa>]
									</div>
				...
				<cfif isuserinrole('admin') and not attributes.comments[k].published>
					...
					<div <cfif attributes.comments[k].private is 'false'>class="commentBody"<cfelse>class="commentBodyPrivate"</cfif>>
						...
						<div class="commentDate">#right(date,2)# #lsdateformat(createdate(2000,mid(date,5,2),1),'mmmm')# #left(date,4)# #attributes.comments[k].time#
							<cfif isuserinrole('admin')>
								[<a href="#request.blogmapping#index.cfm?mode=deletecomment&amp;idcomment=#urlencodedformat(attributes.comments[k].id)#&amp;id=#urlEncodedFormat(attributes.blogid)#">#application.language.language.delete.xmltext#</a>]
								[<a href="#request.blogmapping#index.cfm?mode=viewcomment&amp;idcomment=#urlencodedformat(attributes.comments[k].id)#&amp;id=#urlEncodedFormat(attributes.blogid)#&amp;publish=#attributes.comments[k].published#"><cfif attributes.comments[k].published>#application.language.language.notpublished.xmltext#<cfelse>#application.language.language.published.xmltext#</cfif></a>]
							</cfif>
						</div>
		...
		<cfoutput>
			<cfhtmlhead text="
				<script type=""text/javascript"">
				  var blogTool              = ""AVBlog"";
				  var blogURL               = ""http://#cgi.server_name##request.blogmapping#index.cfm"";
				...
				function verifySubmit()
					{
						<cfif not (isuserinrole('admin') or isuserinrole('blogger')) and useajax()>
							if (_CF_checkaddcomment(window.document.addcomment))
								{
									dojo.io.bind({
										url: "<cfoutput>#request.blogmapping#</cfoutput>ajax.cfm?mode=verifyCaptcha&text=" + window.document.addcomment.captcha.value,
										load: 
			...
			function verifyCaptcha()
				{
					...
					<cfif useajax()>
						<cfoutput>
							...
							TagPane.setUrl('#request.blogmapping#ajax.cfm?mode=verifyCaptcha&text='+window.document.addcomment.captcha.value);
						</cfoutput>
					</cfif>
					return returnValue;
				}
