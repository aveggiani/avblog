	...
	<cfswitch expression="#url.mode#">
		...
		<cfcase value="entry">
			...
			<cfif application.configuration.config.options.trackbacks.xmltext and trim(form.pingTrackBack) is not "">
				<cfscript>
					trackbackresult 				= request.trackbacks.ping(form.pingtrackback,'http://#cgi.server_name##request.blogmapping#index.cfm?mode=viewentry&id=#id#',form.title,form.fckexcerpt,application.configuration.config.headers.title.xmltext);
					structLogValue  				= structnew();
		...
		<cfcase value="users">
			<cfif isdefined('form.addUser') and isuserinrole('admin')>
				<cfscript>
					if (isdefined('form.personalblog'))
						form.personalblog = 'true';
					else
						form.personalblog = 'false';
					request.users.saveUser(form.fullname,form.email,form.us,form.pwd,form.role,form.personalblog,form.blogaddress,form.description);
			...
			<cfif isdefined('form.updateUser') and isuserinrole('admin')>
				<cfscript>
					if (isdefined('form.personalblog'))
						...
						form.personalblog = 'false';
					request.users.updateUser(form.id,form.fullname,form.email,form.us,form.pwd,form.role,form.personalblog,form.blogaddress,form.description);
					application.users					= request.users.loadUsers();
				</cfscript>
			</cfif>
